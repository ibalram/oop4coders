...
Using \keyword{struct} instead of \keyword{class}, all members will be public by default.
...
Every public member in Java must be preceded by the keyword \keyword{public}. 
...
A method defined without a visibility modifier or a field defined using \keyword{var} are public by default. 
...
By default methods are public except for \keyword{initialize} method and the global methods defined under the object class which are always private. 
If desired, you can use the keyword \keyword{public} in a line and all methods defined after it will be public. 
...
Perl can capture the caller (the location or package where a method was called) using the keyword \keyword{caller}. 
...
Methods are public by default, but can be defined as protected using either \keyword{private} or \keyword{protected} keywords. 
...
Methods defined using \keyword{private} or \keyword{protected} can both be accessed from sub-classes.
...
In the contrary, those with \keyword{protected} can be accessed from the an object of the same class.
...
Another way is to use closures (\textit{the combination of a function and the lexical environment within which that function was declared}), by defining a field inside the constructor using the keyword \keyword{var} or \keyword{let} \citep{2001-crockford}.
...
For methods, you can use \keyword{caller}, \keyword{eq}, \keyword{\_\_PACKAGE\_\_} and \keyword{die} to limit access though they still are visible \citep{2006-radiantmatrix}. 
...
They can be declared inside the class header using the keyword \keyword{friend}. 
