...
%Simula history
Simula is considered the first object-oriented object programming language \citep{2004-huang}.
Simula 67 introduced classes, objects, sub-classes, virtual procedures and dynamic binding \cite{2013-black}.
...
By the time, while being at the University of Utah, \nameword{Alan Kay} liked the idea behind Simula language. 
...
Alan Kay defines OOP in term of smalltalk as follows \cite{1993-kay,2014-cunningham}:
\begin{itemize}
...
Since this is a book about OOP, it is a good thing to present quotes opposing it \cite{2016-yegor} so you can have an idea what is going wrong with it. 
There are some concerns about programming languages implementing the OO concepts. 
And then, there are other quotes defending the 
...
We cannot conclude that a paradigm is worse or better than other just because it affords more or less features; ``\textit{More is not better (or worse), just different}"\cite{2004-vanroy-haridi}.
