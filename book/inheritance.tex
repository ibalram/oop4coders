...
If you define a field with the same name as in the superclass, you will still access that of the superclass inside the subclass using the keyword \keyword{super}; this is called \nameword{variable hiding}.
...
Lets define a class by using a function as a constructor (as usual).

\lstinputlisting[language={[KB]Javascript}, linerange={3-11}, style=codeStyle]{../codes/javascript/inheritance/single/es5/person.js}

To call the superclass's constructor, we use the function \keyword{call} which takes the context of the current object \keyword{this} as its first argument. 
...
To call a method from the superclass's prototype, you can use the function \keyword{call} by passing the context as first argument. 
...
A class (package) can extend another using a special variable \keyword{\@ISA}.
...
To call the superclass's methods, you need to use a special keyword \keyword{parent}.
...
Using the keyword \keyword{parent}, you can access just the superclass's methods and not those of grandparent class.
...
A class's meta-class can be set to be \keyword{ABCMeta}, but the class can be instantiated unless there is at least one abstract method.
...
In early C++, a method which is not defined as \keyword{virtual} should not be overridden \citep{2010-torok}. 
...
To fix this problem, the two classes \textbf{Student} and \textbf{Professor} have to be virtual using the keyword \keyword{virtual}.
...
In case of method ambiguity between its parent types (class and/or interfaces), it can access the method of its superclass by simply using \keyword{super}, and the defaults of its parent interfaces by using the name of the interface followed by \keyword{.super.} followed by the name of the method.
...
\begin{itemize}
	...
	Languages affording this capacity are: C++ (class inheritance), Java (interfaces), Perl (\keyword{@ISA}), PHP (interfaces) and Python (class inheritance).
	...
	Languages affording this capacity are: C++ (class inheritance), Java (defaults via interfaces), javascript (merging prototypes), Lua (using \keyword{\_\_index}), Perl (\keyword{@ISA}), PHP (traits), Python (class inheritance) and Ruby (mixins). 
