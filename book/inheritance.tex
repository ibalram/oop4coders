...
C++ supports multiple inheritance: the constructors of inherited classes are called in the same order in which they are inherited. 
...
Also, if your parent classes each has an attribute with the same name, the subclass cannot access it directly by its name since there is a conflict. 
Instead, it must define the attribute of which parent. 
...
Suppose we have a class called \textbf{Person} with an attribute \textbf{name} and the constructor to initialize it. 
This class has two subclasses: \textbf{Student} and \textbf{Professor} which will inherit its attributes and methods. 
...
\lstinputlisting[language={[KB]C++}, linerange={5-8,10-17,20-27,30-38,41-49}, style=codeStyle]{../codes/cpp/inheritance/multiple/diamond2.cpp}
